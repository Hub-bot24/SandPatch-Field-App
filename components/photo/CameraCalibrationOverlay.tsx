...
interface CameraCalibrationOverlayProps {
  photoUrl: string;
  /** Real-world length, in mm, of the ruler whose two ends the operator will tap. */
...
/**
 * One-time camera calibration: the operator taps the two ends of a ruler
 * once, and the resulting pixels-per-mm ratio is reused for every future
 * automatic reading (lib/measurement/autoDetect.ts) - never repeated per
 * photo or per test. This is the one real-world reference the whole
 * automatic-detection feature depends on: a photo alone can never carry an
 * absolute scale on its own.
 */
export function CameraCalibrationOverlay({
  ...
  const pixelsPerMm = isComplete
    ? Math.hypot(taps[1].x - taps[0].x, taps[1].y - taps[0].y) / rulerLengthMm
    : null;
      ...
      <div className="space-y-3 bg-black/90 px-4 py-4 text-white">
        {!isComplete ? (
          <p className="text-center text-base font-semibold">
            {taps.length === 0 ? "Tap one end of your ruler" : `Tap the other end (${rulerLengthMm} mm mark)`}
          </p>
        ) : isDegenerate ? (
